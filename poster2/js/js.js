...
function rsort(n){
	...
	for(var i in pLeft){ //取出左边的所有
		...
		$(photo).css('transform','rotate('+random([-150,150])+'deg) scale(1) translate(500px,0px)');//进阶版 
	}
	for(var i in pRight){
		var photo = pRight[i];
		//$(photo).css('left',random(ranges.right.x)+'px');
		//$(photo).css('top',random(ranges.right.y)+'px');
		$(photo).css('transform','rotate('+random([-150,150])+'deg) scale(1) translate(0px,500px)');//进阶版/**/
	}
